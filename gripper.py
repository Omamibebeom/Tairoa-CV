...
PORT = "COM6"
BAUD = 115200
SERIAL_TIMEOUT = 2    # 讀取逾時(秒)。ACK 只是參考,不能設太長:
                        # 若 Arduino 沒回覆,每個指令會空等滿這個時間。
                        # 之前設 2.0 導致 RELEASE 總耗時 ~7.5s,超過 TM 腳本
                        # 8000ms 讀取窗口,手臂讀不到 $OK 而中断(只夾一個就結束)。
RESET_WAIT = 3.0        # 開埠後等 Arduino 自動重置開機的秒數

USE_ENABLE = False      # True: 用 ENA/ENB 控制出力(需拔 L298N 跳線帽);False: 跳線帽常態致能
SPEED = 1.0             # 馬達出力 0.0~1.0(USE_ENABLE=True 時有效),氣泵通常給滿

# ========================= 動作時間 / 行為 =========================
INFLATE_SEC = 2.5       # 充氣多久才夾緊(依你的氣泵/夾爪實測調整)
DEFLATE_SEC = 1.5       # 洩氣多久才確實鬆開

# HOLD_PRESSURE:
#   False → 充氣 INFLATE_SEC 秒後「斷掉充氣訊號」, 靠止回閥或困住的空氣維持夾持。
#           ★ 目前採用此模式: 持續充氣會過充, 夾爪有裂開風險。
#   True  → 夾取後充氣馬達持續運轉維持壓力, 直到 open() 才停。
#           只適合「會漏氣、且耐壓」的夾爪; 若搬運途中物件會掉, 先考慮
#           調大 INFLATE_SEC 或加止回閥, 而不是貿然改回 True。
HOLD_PRESSURE = True
