...
一轮的流程(手臂先停在 A 区外, 画面干净时):
  ...
  手臂送 GET\r\n    → 吐出佇列中「下一个」, 回:
                      ...
                      佇列空了 → $NONE\r\n
  手臂送 RESET\r\n  → 清空佇列, 回 $OK\r\n
  手臂送 GRIP\r\n   → 樹莓派驱动 L298N「充气马达」夹取(约 1.5s), 完成后回 $OK\r\n
  手臂送 RELEASE\r\n→ 樹莓派驱动 L298N「泄气马达」松开(约 1.2s), 完成后回 $OK\r\n
                      (夹爪不再由手臂 IO 控制, 改由本程式透过 GPIO 控制)
...
import gripper                    # 氣動夾爪(樹莓派 GPIO + L298N)
...
GRIPPER = None       # 氣動夾爪物件, 在 main() 建立(L298N + 充气/泄气马达)
        ...
        while True:
            ...
            while b"\n" in buf:
                ...
                if cmd == "SCAN":
                    ...
                    # 充气马达运转 → 夹爪关闭夹取; 完成(约 INFLATE_SEC 秒)后才回 $OK
                    # 这段是「阻塞」的: 手臂送 GRIP 后会等本程式回 $OK, 确保夹好才移动
                    if GRIPPER is not None:
                        GRIPPER.close()
                    conn.sendall(b"$OK\r\n")
                    print("  GRIP -> closed")
                elif cmd in ("RELEASE", "OPEN"):
                    # 泄气马达运转 → 夹爪松开放下; 完成后回 $OK
                    if GRIPPER is not None:
                        GRIPPER.open()
                    conn.sendall(b"$OK\r\n")
                    print("  RELEASE -> opened")
                else:
                    conn.sendall(b"$ERR\r\n")
    except Exception as e:
...
def main():
    ...
    GRIPPER = gripper.Gripper()          # 初始化 L298N + 充气/泄气马达
    ...
    try:
        ...
        GRIPPER.cleanup()                # 结束时停马达、释放 GPIO
